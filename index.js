...
// Connection URL
const uri = 'mongodb+srv://kaushanthi:chinnu123@clusterkaush.jshvwc0.mongodb.net/universe?retryWrites=true&w=majority';
...
app.get('/comparemore',(req,res)=>{
    const db = client.db(dbName);
    const collection = db.collection('Details');

    collection.find({}).toArray(function(err,docs){
        assert.equal(err, null);
        res.render('comparemore',{'top':docs})
        //res.send(docs);
    });
})

app.get('/comparemore/:id1/:id2/:id3',(req,res)=>{
    const db = client.db(dbName);
    const collection = db.collection('Details');
    ...
    id3 = (hex.test(id3))? ObjectId(id3) : id3;

    collection.find({$or: [{_id : new mongoDB.ObjectID(id1)},
    {_id : new mongoDB.ObjectID(id2)},{_id : new mongoDB.ObjectID(id3)}]}).toArray(function(err,docs){
        assert.equal(err, null);
        res.render('comore',{'top':docs})
    });
})
